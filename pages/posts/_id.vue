...
async function fetchPost(id) {
  const response = await axios.get(`${process.env.API_URL}/wp-json/wp/v2/posts/${id}`);
  return response.data;
}

async function fetchComments(id, page = 1) {
  const response = await axios.get(`${process.env.API_URL}/wp-json/wp/v2/comments/`, {
    params: {
      post: id,
      per_page: 5,
      page,
...
export default {
  ...
  methods: {
    async postComment() {
      ...
      const response = await axios.post(`${process.env.API_URL}/wp-json/wp/v2/comments/`, {
